...
def singleCmsRun(filename, workdir, logdir = None, keep = [], autodelete = [], autodelete_delay = 60., verbose = False, slot = None, executable = 'cmsRun', *args):
  ...
  #print("\n")
  #print("[SINGLE CMSRUN DEBUG] commad to be run:")
  #print(command)
  #print("\n")
  ...
  #print("--------- [DEBUGGING] -------")
  #print("subprocess.Popen(command, cwd = workdir, env = environment, stdout = stdout, stderr = stderr)")
...
def multiCmsRun(
    ...
    verbose = True,                # whether to print extra messages
  ...
  logdir = os.path.abspath(__file__).split("src")[0] + "src/output/logs"
  tmpdir = os.path.abspath(__file__).split("src")[0] + "src/output/tmp"
  #logdir = f"/gpfs/users/blancose/TSGTiming/CMSSW_16_0_0_pre4/src/output/logs"
  #tmpdir = f"/gpfs/users/blancose/TSGTiming/CMSSW_16_0_0_pre4/src/output/tmp"
  ...
  Path(workdir.name).mkdir(parents=True, exist_ok=True)
  
  print("\n")
  print("\n")
  print("<----- Full configuration dump: " + os.path.join(workdir.name, 'process.py'))
  print("\n")
  print("\n")
  ...
  if slots:
    ...
    if set_cpu_affinity:
      ...
      print("\n")
      print("[MULTIRUN CMSRUN] Debugging CPU list:")
      print(cpu_list)
      print("\n")
      
      proc_cpu = "cat /proc/self/status | grep Cpus_allowed_list"
      proc = subprocess.Popen(proc_cpu, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
      out, err = proc.communicate()
      
      out = out.decode("utf-8")
      low_cpu = out.split("\t")[1].split("-")[0]
      low_cpu = int(low_cpu)

      high_cpu = out.split("\t")[1].split("-")[1]
      high_cpu = int(high_cpu.split("\n")[0])

      cpu_list_filtered = []
      for cpu in cpu_list:
          cpu_num = int(cpu)
          if cpu_num>=low_cpu and cpu_num<=high_cpu:
              cpu_list_filtered.append(cpu)
              
      cpu_list = cpu_list_filtered

      print("[MULTIRUN CMSRUN] Debugging filtered CPU list:")
      print(cpu_list)
      print("\n")
    ...
    #print("[MULTIRUN CMSRUN] CPU Assignment:")
    #print(cpu_assignment)
    #print("\n")
    ...
    #print("[MULTIRUN CMSRUN] GPU Assignment:")
  ...
  if debug_affinity:
    for job,slot in enumerate(slots):
      ...
      print(slot.get_command_line_prefix())
    print()
  ...
  for repeat in iterations:
    ...
    for job in range(jobs):
      ...
      print("[MULTIRUN CMSRUN] DEBUG JOBS")
      print(f"Job number -> {job}")
      print(f"config.name -> {config.name}")
      print(f"workdir -> {jobdir}")
      print(f"logdir -> {thislogdir}")
      print(f"autodelete -> {autodelete}")
      print(f"autodelete_delay -> {autodelete_delay}")
      print(f"verbose -> {verbose}")
      print(f"slot -> {slots[job]}")
